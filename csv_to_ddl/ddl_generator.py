...
class DDLGenerator:
        ...
        for table_name, table_spec in tables_specs.items():
            ...
            if table_spec.normalization_suggestions:
                ...
                for suggestion in table_spec.normalization_suggestions:
                    ddl_parts.append(f"-- [{suggestion.suggestion_type}] {suggestion.table_name}: {suggestion.description}")
                    if hasattr(suggestion, 'confidence') and suggestion.confidence:
                        ddl_parts.append(f"--   Confidence: {suggestion.confidence:.1f}\n")
    ...
    def _generate_column_ddl(self, col: ColumnSpec) -> str:
        ...
        sql_type = format_type_with_size(col.data_type, col.size_spec, self.dialect)
