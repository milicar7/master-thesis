...
def format_type_with_size(data_type: DataType, size_spec: ColumnSizeSpec, dialect: DatabaseDialect) -> str:
    ...
    if data_type in [DataType.VARCHAR, DataType.CHAR, DataType.EMAIL] and size_spec.length:
    ...
    elif data_type == DataType.UUID and not config.supports_uuid and size_spec.length:
        return f"{base_type}({size_spec.length})"
